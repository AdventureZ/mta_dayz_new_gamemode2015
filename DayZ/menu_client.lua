...
if not guiGetVisible(spalteGuiImage[2]) and not guiGetVisible(spalteGuiImage[3]) then
return
end
if arg == "up" then
...
guiLabelSetColor (spalteGuiText[1],255,0,0)
guiLabelSetColor (spalteGuiText[2],255,0,0)
...
guiLabelSetColor (spalteGuiText[2],255,0,0)
guiLabelSetColor (spalteGuiText[3],255,0,0)
...
if ( keyState == "down" ) then
	if not guiGetVisible(spalteGuiText[1]) then return end
		...
		if getElementData(getLocalPlayer(),"Shovel") >=1 then
			...
			triggerServerEvent("onPlayerStartAnim",getLocalPlayer())
			outputChatBox("You Hide Body With Success")
			triggerEvent ("displayClientInfo",getLocalPlayer(),"hidebody","You Hide Body With Success!",22,255,0)
			disableMenu()
			return
		end
	end
		...
		if itemName == "patrolstation" then
			...
			if itemName == "Assault Pack (ACU)" or itemName == "Child Briefcase" or itemName == "Alice Pack" or itemName == "Czech Backpack" or itemName == "Mountain Backpack" or itemName == "O.S Pack" or itemName == "LHT Ghillie Pack" or itemName == "Krysset Backpack" or itemName == "Anarchy Assault Pack" or itemName == "Coyote Backpack" then
				local col = getElementData(getLocalPlayer(),"currentCol")
				triggerServerEvent("onPlayerTakeItemFromGround",getLocalPlayer(),itemName,col)
				disableMenu()
				return
