...
--Camu
snipertxd = engineLoadTXD ("mods/soldier_clothing.txd");
engineImportTXD (snipertxd, 287);
sniperdff = engineLoadDFF ("mods/soldier_clothing.dff", 287);
engineReplaceModel (sniperdff, 287);

--Items
-- [NEW] Backpacks ~ começo
itemTXD = engineLoadTXD ("items/child_briefcase.txd");
engineImportTXD (itemTXD, 1877);
itemDFF = engineLoadDFF ("items/child_briefcase.dff", 1877);
engineReplaceModel (itemDFF, 1877);

itemTXD = engineLoadTXD ("items/mountain_backpackGreen.txd");
engineImportTXD (itemTXD, 1878);
itemDFF = engineLoadDFF ("items/mountain_backpackGreen.dff", 1878);
engineReplaceModel (itemDFF, 1878);

itemTXD = engineLoadTXD ("items/backpack_OSPack.txd");
engineImportTXD (itemTXD, 1879);
itemDFF = engineLoadDFF ("items/backpack_OSPack.dff", 1879);
engineReplaceModel (itemDFF, 1879);

itemTXD = engineLoadTXD ("items/backpack_ghillie.txd");
engineImportTXD (itemTXD, 1880);
itemDFF = engineLoadDFF ("items/backpack_ghillie.dff", 1880);
engineReplaceModel (itemDFF, 1880);

itemTXD = engineLoadTXD ("items/backpack_kry.txd");
engineImportTXD (itemTXD, 1881);
itemDFF = engineLoadDFF ("items/backpack_kry.dff", 1881);
engineReplaceModel (itemDFF, 1881);

itemTXD = engineLoadTXD ("items/backpack_anarchy.txd");
engineImportTXD (itemTXD, 1882);
itemDFF = engineLoadDFF ("items/backpack_anarchy.dff", 1882);
engineReplaceModel (itemDFF, 1882);
-- [NEW] Backpakcs ~ fim
-- [Skins]
--Antidon
...
--[[
-- Pistol -> M911
weaponTXD = engineLoadTXD ("items/G13.txd");
engineImportTXD (weaponTXD, 346);
weaponDFF = engineLoadDFF ("items/G13.dff", 346);
	...
	for i,weapon2 in ipairs(damageTable) do
		...
		if weapon1 == weapon then
			if getElementData(getLocalPlayer(),"humanity") == 5000 then
				if weapon2[1] == "G13" or weapon2[1] == "M9 SD" or weapon2[1] == "PPK" then
...
function playerGetDamageDayZ ( attacker, weapon, bodypart, loss )
	...
	if getElementData(attacker,"zombie") then
        ...
        local aleatory = math.random(1,4)
        if aleatory == 3 then
...
if getElementData(getLocalPlayer(),"logedin") then
	...
	--INFECTION
    if getElementData(getLocalPlayer(), "infection") then
        dxDrawImage ( screenWidth*0.94 , screenHeight*0.85, screenHeight*0.065, screenHeight*0.065, "images/dayzicons/infection.png",0,0,0,tocolor(255,255,255))
    end
	--bandit	
...
function infection(source)
    if getElementData(getLocalPlayer(), "logedin") then
        if getElementData(getLocalPlayer(), "infection") then
            ...
            local aleatorySound6 = math.random(1,4)
...
function showDayZDeathScreen()
	...
	deathText = guiCreateLabel(0,0.8,1,0.2,"Voce Morreu! \n Aguarde 5 seconds.",true)
...
function showWhiteScreen ( attacker, weapon, bodypart ) 
    ...
    setTimer(stopSound,2000,1,heart) 
...
function playerPingCheck ()
	if getPlayerPing(getLocalPlayer()) > gameplayVariables["ping"] then
		...
		if pingFails == 5 then
			...
			startRollMessage2("Ping", "Seu ping esta alto demais!PING: "..gameplayVariables["ping"].."! ("..pingFails.."/5)", 255, 22, 0 )
...
dayzVersion = "CassioAMD MTA:DayZ"
